...
def main(config: Config, db: DatabaseDatabase) -> None:
	...
	_check_missing_stream_info(config=config, db=db, update_db=not config.debug)
	# Check for new show scores
	if config.record_scores:
		_check_new_episode_scores(config=config, db=db, update_db=not config.debug)
	# Record poll scores to avoid querying them every time
	_record_poll_scores(db=db, update_db=not config.debug)
	# Show lengths aren't always known at the start of the season
	_check_show_lengths(config=config, db=db, update_db=not config.debug)
	# Check if shows have finished and disable them if they have
	_disable_finished_shows(db=db, update_db=not config.debug)
	...
	for show in shows:
		...
		if latest_episode is not None and 0 < show.length <= latest_episode.number:
			...
			if latest_episode.number > show.length:
				logger.warning(
					"    Episode number (%s) greater than show length (%s)",
...
def _check_missing_stream_info(
	...
	for stream in streams:
		...
		if not service_info:
			logger.warning("No info for service %s", stream.service)
			continue
		logger.info(
		...
		if not service:
			logger.warning("No service associated to the key %s", service_info.key)
			continue
...
def _check_new_episode_scores(
	config: Config, db: DatabaseDatabase, update_db: bool
) -> None:
...
def _record_poll_scores(db: DatabaseDatabase, update_db: bool) -> None:
	...
	for poll in polls:
		if timedelta(days=8) < datetime.now() - poll.date < timedelta(days=93):
			...
			logger.info(
				"Updating poll score for show %s / episode %s (%s)",
				poll.show_id,
				poll.episode,
				score,
			)
	...
	logger.info(
		"%s scores recorded, %s scores not updated", updated, len(polls) - updated
	)
