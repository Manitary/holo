...
class Show(DbEqMixin):
	...
	def __str__(self):
		return "Show: {} (id={}, type={}, len={})".format(
			self.name, self.id, self.type, self.length
		)
...
class Episode:
	...
	def __str__(self):
		return "Episode: {} | Episode {}, {} ({})".format(
			self.date, self.number, self.name, self.link
		)
...
class UnprocessedStream:
	def __init__(
		self, service_key, show_key, show_id, name, remote_offset, display_offset
	):
