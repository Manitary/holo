...
class PollHandler(AbstractPollHandler):
    ...
    def create_poll(
        ...
        try:
            form = requests.get(self._poll_post_url, timeout=self.default_timeout)
        except Exception as e:
            logger.error("Could not obtain a form to fill: %s", e)
            return None
        ...
        try:
            ...
            logger.error("Could not retrieve the form token: %s", e)
            return None
        ...
        try:
            ...
            logger.error("Could not retrieve the session cookie: %s", e)
            return None

        # Submit poll creation form
        data = self._poll_post_data.format(
            token=token, question=title.replace('"', '\\"')
        )
        try:
            resp = requests.post(
                self._poll_post_url,
        ...
        logger.error("Could not create poll (ID not found)")
